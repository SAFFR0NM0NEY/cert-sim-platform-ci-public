export default function DropdownCommandQuestion({
  question,
  answer,
  onAnswerChange,
}) {
  ...
  return (
    <div className="interactive-question">
      <p className="interaction-hint">Complete the command with the correct values.</p>
      {!isComplete && (
      ...
      <pre className="command-block">
        <code>
          {renderTemplate(
            question.commandTemplate ?? question.codeTemplate,
            question,
            answer,
            onAnswerChange,
          )}
...
function renderTemplate(template, question, answer, onAnswerChange) {
  ...
  return template.split(/({{[^}]+}})/g).map((part, index) => {
    ...
    return (
      <select
        className="inline-dropdown command-dropdown"
        key={`${blank.id}-${index}`}
