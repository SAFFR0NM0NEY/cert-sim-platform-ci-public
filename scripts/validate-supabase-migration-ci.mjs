...
try {
  ...
  assert.equal(Object.keys(SOURCE_MIGRATION_HASHES).length, 60);
  assert.throws(
  ...
  assert.equal(assignment.files.length, 60);
  ...
  assert.equal(SOURCE_MIGRATIONS.at(-8), '20260903084553_p0_production_recovery_52_57.sql');
  assert.equal(SOURCE_MIGRATIONS.at(-7), '20260903120000_issue59_function_definition_line_ending_preflight.sql');
  assert.equal(SOURCE_MIGRATIONS.at(-6), '20260903161929_issue_59_functional_recovery.sql');
  assert.equal(SOURCE_MIGRATIONS.at(-5), '20260904161938_issue65_full_untimed_practice_sessions.sql');
  assert.equal(SOURCE_MIGRATIONS.at(-4), '20260905090355_issue65_live_acceptance_lifecycle_fix.sql');
  assert.equal(SOURCE_MIGRATIONS.at(-3), '20260905153716_ai901_canonical_form_rotation.sql');
  assert.equal(SOURCE_MIGRATIONS.at(-2), '20260905192938_cross_version_package_compatibility.sql');
  assert.equal(SOURCE_MIGRATIONS.at(-1), '20260906104745_register_sc200_runtime_contract.sql');
