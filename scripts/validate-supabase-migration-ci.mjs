...
try {
  ...
  assert.equal(Object.keys(SOURCE_MIGRATION_HASHES).length, 56);
  assert.throws(
  ...
  assert.equal(assignment.files.length, 56);
  ...
  assert.equal(SOURCE_MIGRATIONS.at(-4), '20260903084553_p0_production_recovery_52_57.sql');
  assert.equal(SOURCE_MIGRATIONS.at(-3), '20260903120000_issue59_function_definition_line_ending_preflight.sql');
  assert.equal(SOURCE_MIGRATIONS.at(-2), '20260903161929_issue_59_functional_recovery.sql');
  assert.equal(SOURCE_MIGRATIONS.at(-1), '20260904161938_issue65_full_untimed_practice_sessions.sql');
