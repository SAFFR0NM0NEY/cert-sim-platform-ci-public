...
const assignedRequest={...baseRequest,assignmentId,clientRequestId:crypto.randomUUID()};
const assigned=await admin.rpc('certsim_protected_start_practice',{p_actor_id:learner.id,p_request:assignedRequest});
...
if(assignedReview.error||assignedReview.data?.code!=='review_unavailable') fail('ASSIGNED_REVIEW_RELEASE_NOT_WITHHELD');
...
function createAttempt(profile,purpose,status='in_progress'){const id=crypto.randomUUID(),request=crypto.randomUUID();sql(`insert into exam_delivery.attempts(id,owner_id,package_version_id,package_profile_id,client_request_id,status,purpose,generator_version,scorer_version,created_at,started_at,expires_at) values ('${id}','${lowlevel.id}','${packageId}','${profile}','${request}','${status}','${purpose}','certsim-ai901-weighted-generator-v2','certsim-ai901-exact-scorer-v2',now(),now(),now()+interval '10 minutes')`);return{id,request}}
