...
for(const [label,mutate] of [
  ...
  ['extra-form-key',(p)=>{p.profiles[0].selection.canonicalForms.forms[0].rawMaximum=2}],
  ['missing-skill-targets',(p)=>{delete p.profiles[0].selection.canonicalForms.skillGroupTargets}],
  ['missing-required-objectives',(p)=>{delete p.profiles[0].selection.canonicalForms.requiredObjectiveKeys}],
  ['missing-coverage-requirements',(p)=>{delete p.profiles[0].selection.canonicalForms.minimumCoverageTagCounts}],
  ['missing-difficulty-requirements',(p)=>{delete p.profiles[0].selection.canonicalForms.difficultyRequirements}],
  ['wrong-fallback-policy',(p)=>{p.profiles[0].selection.canonicalForms.reservePolicy='none-disjoint-full-bank'}],
  ['missing-formal-release-policy',(p)=>{delete p.profiles[0].selection.formalReleasePolicy}],
  ['wrong-target-type',(p)=>{p.profiles[0].selection.canonicalForms.skillGroupTargets['synthetic-skill']='2'}],
...
function assertStarted(result,count,label){if(result.error||result.data?.ok!==true||result.data?.items?.length!==count)fail(`${label}_FAILED_${safeDatabaseIdentifier(result.error?.message)}_${safeDatabaseIdentifier(result.data?.code)}_${result.data?.items?.length??'none'}`)}
