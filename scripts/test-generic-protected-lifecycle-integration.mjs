...
if(assignedReplacement.error||assignedReplacement.data?.ok!==true||assignedReplacement.data?.attempt?.attemptId===assignedStarted.data.attempt.attemptId) fail(`ASSIGNMENT_REPLACEMENT_FAILED_${safeCode(assignedReplacement.error?.code)}_${safeDatabaseIdentifier(assignedReplacement.error?.message)}_${safeCode(assignedReplacement.data?.code)}`);
...
sql(`alter table exam_delivery.attempts disable trigger guard_attempt_identity_and_lifecycle; update exam_delivery.attempts set created_at=statement_timestamp()-interval '31 days',started_at=statement_timestamp()-interval '31 days',practice_last_activity_at=statement_timestamp()-interval '31 days',practice_idle_expires_at=statement_timestamp()-interval '1 minute' where id='${weakStaleId}'::uuid; alter table exam_delivery.attempts enable trigger guard_attempt_identity_and_lifecycle;`);
...
for(const config of [
  {purpose:'study_sandbox',language:'not_applicable',check:true},
  {purpose:'targeted_domain',language:'not_applicable',domain:'d1'},
  {purpose:'pbq_practice',language:'not_applicable'},
]){
