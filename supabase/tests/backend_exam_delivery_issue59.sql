...
select has_function('exam_delivery','reconcile_expired_formal_attempts',array['uuid','uuid','text','text']);
...
select ok(not has_function_privilege('authenticated','exam_delivery.reconcile_expired_formal_attempts(uuid,uuid,text,text)','EXECUTE'),'browser cannot invoke stale formal reconciliation');
...
select ok((select pg_get_functiondef(p.oid) ~ 'prior.package_version_id=v_attempt.package_version_id' from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='exam_delivery' and p.proname='materialize_attempt_items_issue21_unrotated_base'),'delegated practice materializer uses same-package evidence across profiles');
