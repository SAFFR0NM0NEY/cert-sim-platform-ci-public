...
begin
  ...
  if found then
    if case when v_existing.purpose in ('study_sandbox','targeted_domain','weak_area','pbq_practice')
      then v_existing.practice_idle_expires_at>v_now else v_existing.expires_at>v_now end
  ...
  if v_has_expired then
    update exam_delivery.attempts set status='expired' where id=v_existing.id and status='in_progress'
      and case when v_existing.purpose in ('study_sandbox','targeted_domain','weak_area','pbq_practice')
        then practice_idle_expires_at<=v_now else expires_at<=v_now end;
